...
set OPENSSL_PATH1=C:\Program Files\OpenSSL
set OPENSSL_PATH2=C:\Program Files\OpenSSL-Win64

if exist "%OPENSSL_PATH1%\libssl-1_1-x64.dll" (
    set "OPENSSL_PATH=%OPENSSL_PATH1%"
) else if exist "%OPENSSL_PATH1%\bin\libssl-1_1-x64.dll" (
    set "OPENSSL_PATH=%OPENSSL_PATH1%\bin"
) else if exist "%OPENSSL_PATH2%\libssl-1_1-x64.dll" (
    set "OPENSSL_PATH=%OPENSSL_PATH2%"
) else if exist "%OPENSSL_PATH2%\bin\libssl-1_1-x64.dll" (
    set "OPENSSL_PATH=%OPENSSL_PATH2%\bin"
) else (
    echo ERROR: Cannot find OpenSSL DLLs
    exit /B 1
)
echo Using OpenSSL from "%OPENSSL_PATH%"
copy "%OPENSSL_PATH%\libssl-1_1-x64.dll" "%TARGET%\" || ( call :setESC & echo. & echo. & echo %ESC%[91mBuild FAILED.%ESC%[0m  & EXIT /B 1)
copy "%OPENSSL_PATH%\libcrypto-1_1-x64.dll" "%TARGET%\" || ( call :setESC & echo. & echo. & echo %ESC%[91mBuild FAILED.%ESC%[0m  & EXIT /B 1)
...
"c:\Program Files (x86)\Inno Setup 6"\iscc.exe "/dMyAppVersion=%VERSION%" "/dMyAppId={{0AF9BF43-8D44-4AFF-AE60-6CECF1BF0D31}" "/dMyAppDir=rclone-browser-%VERSION_COMMIT%-windows-64-bit" "/dMyAppArch=x64os" /O"%~dp0\..\release" /F"rclone-browser-%VERSION_COMMIT%-windows-64-bit" "%~dp0\rclone-browser-win-installer.iss" || ( call :setESC & echo. & echo. & echo %ESC%[91mBuild FAILED.%ESC%[0m  & EXIT /B 1)

echo.> "%TARGET%\RcloneBrowser.ini" || ( call :setESC & echo. & echo. & echo %ESC%[91mBuild FAILED.%ESC%[0m  & EXIT /B 1)
